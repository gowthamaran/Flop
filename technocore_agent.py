    ...
    *,
    ...
    request = Request(
        ...
        headers={
            ...
            "User-Agent": f"flop-technocore-starter/{APP_VERSION}",
        },
    )
...
def build_parser() -> argparse.ArgumentParser:
    ...
    doctor_parser = commands.add_parser(
        "doctor", help="print machine-readable local readiness checks"
    )
...
def run_command(args: argparse.Namespace) -> int:
    ...
    if args.command == "doctor":
        resolved_key = args.key.expanduser().resolve()
        report = {
            "app_version": APP_VERSION,
            "base_url": validate_base_url(args.base_url),
            "identity_exists": resolved_key.is_file(),
            "identity_path": str(resolved_key),
            "python": ".".join(str(part) for part in sys.version_info[:3]),
            "python_supported": sys.version_info >= (3, 12),
            "ready_for_init": not resolved_key.exists(),
        }
